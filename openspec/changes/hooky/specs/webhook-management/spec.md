...
Webhook Management provides the ability to create, list, and delete webhook endpoints. Each webhook is identified by a unique token that can be auto-generated or user-defined. Management features (create, list, delete, enable/disable) are only available to authenticated users. Anonymous users receive an auto-generated webhook on homepage visit and can only inspect requests.
