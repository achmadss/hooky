...
Authentication provides anonymous/guest mode and user account functionality. Anonymous users are instantly provided with a webhook on homepage visit and redirected to the detail page for immediate use, while authenticated users see the full management dashboard with persistent storage. The system supports a webhook claiming workflow when anonymous users decide to log in.
